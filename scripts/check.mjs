...
const validateSources = (sources, label) => {
  if (!Array.isArray(sources) || sources.length === 0) fail(`${label}: no sources`);
  const sourceIds = new Set();
  for (const source of sources) {
    if (!source?.id || !source?.name) fail(`${label}: source missing id/name`);
...
for (const date of historyIndex) {
  ...
  validateSources(snapshot.sources, `history/${date}.json`);
}
